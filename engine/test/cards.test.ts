...
test("SuitedConsecutiveCardSet", () => {
    ...
    expect(cards2.front()).toBe(Cards._5h)
    expect(cards2.back()).toBe(Cards._4h)

    expect(cards1.merge(cards2)).toBeTrue()
    expect(cards1.size()).toBe(6)
    expect(cards1.merge(cards2)).toBeFalse()
    expect(cards1.size()).toBe(6)
    expect(cards1.front()).toBe(Cards._Kh)
    ...
    const cards5 = new SuitedConsecutiveCardSet(Cards._Jc, false)
    const cards6 = new SuitedConsecutiveCardSet(Cards._9c, false)
    expect(cards5.add(Cards._Tc)).toBeTrue()
    expect(cards6.add(Cards._8c)).toBeTrue()
    expect(cards6.merge(cards5)).toBeFalse()
    expect(cards5.size()).toBe(2)
    expect(cards6.size()).toBe(2)
    expect(cards5.merge(cards6)).toBeTrue()
    expect(cards5.front()).toBe(Cards._Jc)
    expect(cards5.back()).toBe(Cards._8c)
})
